...
const load_stats = () => {
    ...
    d3.json(path_to_stats_json).then(stats => {
        render_stats(stats)
...
const render_stats = (stats_obj) => {
    ...
    last_update = moment.unix(stats_obj.last_update)
                        .local()
                        .format('MMM Do, HH:mm:ss');
    last_update_html = `Last update: ${last_update} (Refreshes every ${auto_refresh_interval/1000})s`;
