...
emoji_files = [x for x in os.listdir(custom_emoji_dir) if x.endswith('.png') or x.endswith('.jpg') or x.endswith('.gif')]
...
for emoji_f in emoji_files:
	...
	try:
		response = requests.post(config["server"]+API_path+'emoji-custom.create', headers=headers, files=files)
