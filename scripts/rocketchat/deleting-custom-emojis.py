...
response = requests.get(config["server"]+API_path+'emoji-custom.list', headers=headers)
...
for emoji_id in current_custom_emojis:
	...
	try:
		response = requests.post(config["server"]+API_path+'emoji-custom.delete', headers=headers, data=data)
